        ...
        if (ctx->currjob != NULL) {
            ...
            if (ctx->currjob->status == ASYNC_JOB_PAUSING) {
                *job = ctx->currjob;
				...
				{
					ctx->currjob->status = ASYNC_JOB_PAUSED;	//inner fibre
					ctx->currjob = ctx_cp->currjob;
					ctx->currjob->status = ASYNC_JOB_RUNNING;	//outer fibre
					ctx->dispatcher = ctx_cp->dispatcher;
					free(ctx_cp);
				}
            ...
            //*job = NULL;
            //return ASYNC_ERR;
            ctx_cp = (async_ctx *)malloc(sizeof(async_ctx));
            *ctx_cp = *ctx;
