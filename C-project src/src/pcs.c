...
pcs_private_key* pcs_init_private_key(void)
{
    ...
    mpz_inits(vk->p, vk->p2, vk->q, vk->q2, vk->hp, vk->hq, vk->mu,
              vk->lambda, vk->n, vk->n2, vk->qtmp, vk->qinv, NULL);
    return vk;
}

pcs_private_key_py* pcs_init_private_key_py(void)
{
    pcs_private_key_py *vk1 = malloc(sizeof(pcs_private_key_py));
    if (!vk1) return NULL;
    mpz_inits(vk1->p2, vk1->q2, vk1->qinv, NULL);
    return vk1;
}
                           ...
                           hcs_random *hr, const unsigned long bits)
{
    ...
    //mpz_mod(vk->qtmp, vk->q2, vk->p2);
    mpz_invert(vk->qinv, vk->q2, vk->p2);
...
void pcs_encrypt_test(pcs_public_key *pk, hcs_random *hr, mpz_t rop, mpz_t plain1)
{
	mpz_t t_test;
	mpz_init(t_test);
...
#pragma omp section
		{
			mpz_random_in_mult_group(t_test, hr->rstate, pk->n);
			mpz_powm(t_test, t_test, pk->n, pk->n2);
		}
#pragma omp section
		{
			mpz_powm(rop, pk->g, plain1, pk->n2);
...
void pcs_encrypt(pcs_public_key *pk, hcs_random *hr, mpz_t rop, mpz_t plain1, CpaInstanceHandle* pCyInstHandle)
{
    ...
    #pragma omp parallel sections
    {
        #pragma omp section
        {
            ...
            // mpz_mod(q_tmp,vk->q2,vk->p2);
            // mpz_invert(q_tmp,q_tmp,vk->p2);
            // PowModN()
        ...
        #pragma omp section
        {
            mpz_mul(rop, pk->n, plain1);
            mpz_add_ui(rop, rop, 1);
            mpz_mod(rop, rop, pk->n2);
        }
    }
    // ASYNC_start_job(&job, wctx, &ret, AsyncPowModN, pArg, sizeof(struct elem_PowModN));
    // mpz_mul(rop, pk->n, plain1);
    // mpz_add_ui(rop, rop, 1);
    // mpz_mod(rop, rop, pk->n2);
...
void pcs_encrypt_crt_py(pcs_public_key *pk, pcs_private_key_py *vk1, hcs_random *hr, mpz_t rop, mpz_t plain1, CpaInstanceHandle* pCyInstHandle)
{
	mpz_t t1,t2,t3;
    ...
    #pragma omp parallel sections
    {
        ...
        {
            mpz_random_in_mult_group(t1, hr->rstate, pk->n);
            PowModN(&t2, &t1, &(pk->n), &(vk1->p2), pCyInstHandle);
            PowModN(&t3, &t1, &(pk->n), &(vk1->q2), pCyInstHandle);
            mpz_sub(t1,t2,t3);
            mpz_mul(t1,vk1->qinv,t1);
            mpz_mod(t1,t1,vk1->p2);
            mpz_mul(t1,t1,vk1->q2);
            mpz_add(t1,t1,t3);
            mpz_mod(t1,t1,pk->n2);

            // PowModN(&t1, &t1, &(pk->n), &(vk->p2), pCyInstHandle);
            // mpz_sub_ui(t3,t1,50);
            // mpz_sub(t1,t2,t3);
            // mpz_mul(t1,vk->qinv,t1);
            // mpz_mod(t1,t1,vk->p2);
            // mpz_mul(t1,t1,vk->q2);
            // mpz_add(t1,t1,t3);
            // mpz_mod(t1,t1,pk->n2);
...
void pcs_encrypt_crt(pcs_public_key *pk, pcs_private_key *vk, hcs_random *hr, mpz_t rop, mpz_t plain1, CpaInstanceHandle* pCyInstHandle)
{
	mpz_t t1,t2,t3;
	mpz_inits(t1,t2,t3,NULL);
    ...
    #pragma omp parallel sections
    {
        ...
        {
            mpz_random_in_mult_group(t1, hr->rstate, pk->n);
            PowModN(&t2, &t1, &(pk->n), &(vk->p2), pCyInstHandle);
            // gmp_printf("t2 = %Zd\n", t2);
            // printf("second powmodn start\n");
            PowModN(&t3, &t1, &(pk->n), &(vk->q2), pCyInstHandle);
            mpz_sub(t1,t2,t3);
            mpz_mul(t1,vk->qinv,t1);
            mpz_mod(t1,t1,vk->p2);
            mpz_mul(t1,t1,vk->q2);
